...
isAlive : Model -> Bool
isAlive model =
    not (List.any (isIntersecting model.cell) model.boots)


isIntersecting : Cell -> Boot -> Bool
...
updateBoots : Model -> Float -> List Boot
updateBoots model dt =
    List.map (updateBoot dt) model.boots


updateBoot : Float -> Boot -> Boot
updateBoot dt boot =
    let
        newVelocity =
...
viewBoot : Boot -> Html Msg
viewBoot boot =
    let
        x =
            200 + boot.radius * cos (boot.angle)

        y =
            200 + boot.radius * sin (boot.angle)
    in
